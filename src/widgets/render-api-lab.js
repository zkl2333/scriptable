...
const makeGradient = (colors, start, end) => {
...
const addHome = (widget) => {
  ...
  status.addSpacer(5);
  const time = status.addDate(new Date(Date.now() + 52 * 60 * 1000));
  ...
  if (family !== 'small') {
    const dates = widget.addStack();
    dates.centerAlignContent();
    const relative = dates.addDate(new Date(Date.now() + 2 * 60 * 60 * 1000));
    relative.font = Font.regularSystemFont(10);
    relative.textColor = palette.muted;
    relative.applyRelativeStyle();
    dates.addSpacer();
    const offset = dates.addDate(new Date(Date.now() - 30 * 60 * 1000));
  ...
  if (family === 'large' || family === 'extraLarge') {
    widget.addSpacer();
    const foot = widget.addText('Text · Date · Image · Stack · Gradient');
