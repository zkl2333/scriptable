...
const C = {
  ...
  led: new Color("#d97706"), // 位图无法动态换色，选取兼顾米白与黑底的深琥珀
...
function renderMedium(w, data, time) {
  ...
  lab.textColor = C.dim;
  left.addSpacer(3);
  addLed(left, money(data.today_cost), { dot: 2.4, gap: 1.0 }, 148);
  left.addSpacer(3);
...
function renderAccessory(w, family, data) {
  ...
  const circularCost = numericCost >= 1000
    ? `${(numericCost / 1000).toFixed(numericCost >= 10000 ? 0 : 1)}K`
    : cost;
